  ...
  >
    ...
    <div class="register-provider-section">
      ...
      <div class="register-form-grid register-form-grid--two">
        ...
        <label v-if="currentType === 'cloudflare_temp_email'" class="register-field">
          <span class="register-label">邮件列表路径</span>
          <Input
            :model-value="provider.messages_path"
            block
            root-class="font-mono"
            :disabled="disabled"
            placeholder="/admin/mails"
            @update:model-value="value => emit('update-field', index, 'messages_path', String(value || '').trim())"
          />
        </label>

        <label v-if="currentType === 'cloudflare_temp_email'" class="register-field">
          <span class="register-label">查信鉴权</span>
          <GroupedSelectMenu
            :model-value="provider.messages_auth"
            :groups="messagesAuthGroups"
            selected-indicator="none"
            :disabled="disabled"
            block
            @update:model-value="value => emit('update-field', index, 'messages_auth', value)"
          />
        </label>
